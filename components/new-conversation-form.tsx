...
export default function NewConversationForm({
  ...
  // Register selectedUsers manually
  ...
  const processForm: SubmitHandler<Inputs> = async data => {
    ...
    const { name, imageUrl, selectedUsers } = data

    // === Sanitize imageUrl ===
    let safeImageUrl = ''
    if (imageUrl && imageUrl.length < 500 && imageUrl.startsWith('http')) {
    ...
    const safeData = {
      name,
      selectedUsers,
      ...(safeImageUrl && { imageUrl: safeImageUrl }) // only include imageUrl if valid
    }

    console.log('Sending to server:', safeData)

    const result = await createNewConversationAction(safeData)
